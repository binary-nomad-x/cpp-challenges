...
void printSingleTable(const int number, const int limit) {
    cout << "\n";
...
int main() {
    ...
    while (running) {
        ...
        const int choice = getValidInt("");

        if (choice == 1) {
            // --- Option 1: Single Table ---
            const int number = getValidInt("Enter the number: ");
            const int limit = getValidInt("Enter the limit (e.g., 10): ");
            ...
            const int limit = getValidInt("Enter limit (e.g., 10): ");
